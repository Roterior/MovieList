...
public class ReviewController {
    ...
    @PUT
    //@Path("/view") // Example: http://localhost:8081/review/view/edit?rId=100&description=textTest&rating=4.9
    ...
    public Review add(@QueryParam("imdbId") String imdbId, @QueryParam("description") String description,
                      @QueryParam("rating") double rating, @Context HttpServletRequest req) {
        HttpSession session = req.getSession();
        String login = session.getAttribute("login").toString();
